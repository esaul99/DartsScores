...
function updateScore(score) {
    if (currentPlayer === 1) {
        ...
        if (player1Score <= 0) {
            document.getElementById('messageArea').innerText = `${document.getElementById('player1').textContent.split(':')[0]} wins!`;
            playWinSound();
        } else {
            currentPlayer = 2;
            toggleDot();
        ...
        if (player2Score <= 0) {
            document.getElementById('messageArea').innerText = `${document.getElementById('player2').textContent.split(':')[0]} wins!`;
            playWinSound();
        } else {
            currentPlayer = 1;
            toggleDot();
